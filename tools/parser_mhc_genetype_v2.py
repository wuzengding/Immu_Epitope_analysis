...
def process_mhc_file_original(file_path, sample_id):
    """
    Original模式：提取Excel中原始的MHC分型数据，不做任何格式转换，原样输出
    """
    # 加载Excel文件
    df = pd.read_excel(file_path)
    ...
    mhc_columns = [
        'A1*', 'A2*', 'B1*', 'B2*', 'C1*', 'C2*',
        'DQA1-1*', 'DQA1-2*', 'DQB1-1*', 'DQB1-2*',
        'DPA1-1*', 'DPA1-2*', 'DPB1-1*', 'DPB1-2*',
        'DRB1-1*', 'DRB1-2*', 'DRB345*-1', 'DRB345*-2'
    ]
    
    # 提取原始值，处理空值
    original_data = {}
    for col in mhc_columns:
        value = row[col].values[0]
        # 处理空值和NaN，保持原始格式
        if pd.isna(value) or value.strip() == '':
            original_data[col] = '无数据'
        else:
            original_data[col] = value.strip()
    
    # 格式化为前端友好的字符串（键值对形式，清晰易读）
    result_lines = [f"样本ID: {sample_id}", "原始MHC分型数据:"]
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--mode', type=str, choices=['SAKit', 'Scanno2', 'original'], 
                        default='SAKit', help='Output mode (SAKit, Scanno2 or original)')
    ...
    if args.show:
        ...
        if args.mode == 'SAKit':
            ...
            result = process_mhc_file_original(args.file_path, args.sample_id)
            print(result)
    else:
